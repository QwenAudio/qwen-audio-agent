...
- 资料库达到容量上限时明确拒绝新增，不再静默移除旧记录；导入与转换命名同时检查
  磁盘已有文件，避免覆盖未索引的旧副本，已收录文本仍可更新。

- 修复桌面安装包遗漏共享运行时模块、打包成功但无法启动的问题；统一开发与发布
  使用的 Electron 版本，并增加 macOS / Windows 成品启动与退出检查；npm 发布等待
  桌面成品通过验收后再执行。
- 修复资料库仅大小写或 Unicode 规范形式不同的文件名互相覆盖的问题，确保导入、
  转换命名与删除保持一致。
- 修复桌面设置保存 Windows 路径后反斜杠、引号被错误解析的问题。
- 修复 Windows 后台启动器对含括号等字符的路径与参数转义错误，以及 OpenClaw
  固定版本包启动依赖 Unix `which` 命令的问题。
