...
- 默认情况下，麦克风音频、实时转写上下文和模型回复请求会发送到阿里云
  ...
  该服务。
- 可选的 Web/TUI composer 听写默认关闭。显式启用后，听写音频只发送到独立的
  Qwen ASR 连接，不会回退到主 Realtime 模型。听写草稿、转写和编辑状态仅存在于
  当前进程与 WebSocket 会话；取消、失败或关闭连接会丢弃未提交内容。
...
听写不会新增历史文件、操作系统级输入权限或跨应用录音。未提交的听写音频与文本
不会写入 conversation、长期记忆、磁盘或日志。只有用户说出独立的中英文终止命令后，
当前草稿才会通过 Web/TUI 原有 composer 提交路径进入正常会话；从该时刻起，它适用
正常会话、后台 Agent 与记忆功能的数据处理规则。开放式改写只进行一次无 tools、
无 memory 的文本请求；精确插入、替换与删除在本地按确定性规则完成。
