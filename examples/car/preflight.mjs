...
export function cockpitEndpoints(env = process.env) {
  return [
    {
      label: 'Domain',
      host: env.COCKPIT_DOMAIN_HOST || '127.0.0.1',
      port: port(env, 'COCKPIT_DOMAIN_PORT', 3_010),
    },
    {
      label: 'Agent',
...
export function assertRealtimeConfigured(env = process.env) {
  ...
  if (provider === 'speech-to-speech') {
    ...
    throw new Error(
      '缺少 SPEECH_TO_SPEECH_REALTIME_URL。请配置 examples/car/.env.local。',
    )
  }
  if (env.QWEN_AUDIO_REALTIME_API_KEY || env.DASHSCOPE_API_KEY) return
  throw new Error(
    '缺少 DASHSCOPE_API_KEY。请复制 examples/car/.env.example 为 '
    + 'examples/car/.env.local，并填写 Key。',
  )
}
