...
- An optional `modelCatalog()` returns the provider's complete model matrix. Every returned
  profile is validated at registration time; `voiceCapabilities` may publish system voices,
  `supportsClonedVoices`, and JSON-safe `customVoicePrefixes` for client-side guidance.
...
Connection-time Provider errors are passed to the Gateway with Provider, model, voice, and
safe capability-list context.
