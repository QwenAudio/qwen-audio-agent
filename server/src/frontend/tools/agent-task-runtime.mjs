...
export class AgentTaskRuntime {
  ...
  async executeSpawnThinkingToolCall({
    ...
    const pendingInputTasks = this.host.taskManager.list({
      ownerId: this.host.ownerId,
      sessionId: this.host.sessionId,
      active: true,
    ...
    if (pendingInputTasks.length) {
      ...
      // from a spawn objective: return the real request so the frontend can
      // forward the customer's response through the existing input protocol.
      await this.host.sendOutput(callId, {
