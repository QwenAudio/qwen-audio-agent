...
后台 Agent 默认在内存中保留最近 50 轮任务请求和最终回复，每次推理最多携带
49 轮历史和当前轮。示例 Gateway 为同一用户的新任务复用服务端返回的 A2A Context；待确认的
操作预览仅属于原 Task，不作为历史重放。重置业务会话、换客户或重启后台 Agent
后重新积累后台历史。这不改变前台既有的聊天记录恢复机制，也不是长期记忆。
