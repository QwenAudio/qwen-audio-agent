...
function agentCard(origin) {
  return {
    name: 'Lightweight Cockpit Agent',
    description: 'Replaceable A2A example for a small set of cockpit operations.',
    supportedInterfaces: [{
      url: `${origin}/`,
...
export async function startCockpitAgentServer({
  host = '127.0.0.1',
  port = 3020,
  domainOrigin = 'http://127.0.0.1:3010',
  cockpitId = 'default',
  tools = new CockpitMcpTools({ origin: domainOrigin, cockpitId }),
} = {}) {
  const card = agentCard(`http://${host}:${port}`)
  const executor = new CockpitAgentExecutor({ tools })
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  const runtime = await startCockpitAgentServer({
    ...
    domainOrigin: process.env.COCKPIT_DOMAIN_ORIGIN || 'http://127.0.0.1:3010',
