...
- 前台支持网页搜索、网页读取、MCP / OpenAPI 工具与资料库检索；复杂工作仍可异步交给
  后台 Agent，统一查询、取消、权限确认与补充输入，不阻塞语音对话。
- 新增桌面对话面板，支持查看历史、输入文字和图片；支持视频模型的 WebUI 可独立开关
  摄像头与麦克风，并在对话中展示后台生成的文件和图片产物。
- 扩展实时前台：新增 Qwen3.8 Omni、MiniCPM-o、StepFun、GPT-Live、Google Live 与
  Doubao Seeduplex 适配；具体音视频及工具能力由各 Provider 和模型决定。
- 新增移动端开发客户端与远程配对，可通过局域网或 Tailscale 私有网络连接电脑上的
  Gateway；桌面端、WebUI 与移动端复用统一连接入口及可单独吊销的设备凭证。
  Android 提供开发 APK，iOS 当前提供模拟器构建，不作为正式应用商店版本发布。
- 完善长期记忆与本机资料库，提供可替换的 Memory / Knowledge Provider；新增
  VoiceMem、Memcode 与 LightRAG 集成示例。
- 新增 WebRTC、X-Omni、智能客服及嵌入式语音设备示例，扩展智能座舱示例；统一
  客户端协议与后台适配边界，支持 ACP、A2A 及自定义后台接入。
- 加固语音重连、工具失败续答、授权、休眠唤醒、音频设备切换、慢连接背压和日志保留；
  发布校验与打包绑定同一提交，增加桌面成品检查。
...
- 修复资料库把已转换的 PDF / Word 再导入当成新文档：达到容量时被拒绝，
  未满时还会留下多余副本。现在按原始文件路径识别同一份资料，更新已收录的
  转换结果，行为与文本再导入一致。

- 修复 Realtime 不回显消息 ID 时并发上下文确认超时；重连后恢复待授权与待补充信息，
  等待用户确认期间不再播报旧的执行进度。

- WebUI 默认跟随浏览器语言，不再受历史本地缓存覆盖，并响应浏览器语言切换。
- 回复占用等待服务端释放；超时后取消并确认，无法确认时重建连接。任务已结束时
  不再播放延迟的过程回执，避免先完成再播报“正在处理”。

- 修复桌面休眠污染连接状态、唤醒后再次休眠失败的问题；面板仅在隐藏成功后切回悬浮球，
  避免隐藏失败时对话内容被压缩进小窗口。休眠和唤醒均静默更新模型上下文，避免唤醒后
  仍把历史休眠回执当作当前状态。
  状态通知区分空闲超时自动休眠与执行休眠请求，保留实际原因。

- 客户端环境信息支持自包含文本投递，无需预注册业务事件；客户端可在握手中声明工具，
  网关统一转发调用。桌面休眠由客户端执行，状态同步与模型上下文投递各自独立。

- DashScope Provider 新增 Qwen3.8 Omni Flash Realtime，支持业务空间专属地址与新版音频配置；
  复用现有语音、图像帧、工具续答及后台结果播报链路，默认模型不变。
- 修复 Realtime 内容拒绝后的重复重连：历史恢复等待服务端确认，拒绝的上下文不再
  反复回灌；连续恢复失败会明确报错并停止自动重试，保留对话记录与后台工作。
- 修复 Realtime 自动回复与工具续答、主动播报相撞时的 pending response 错误：
  发送前重新检查响应状态，冲突后有限重试，不重复执行工具或误清除正在生成的回复。
- WebUI 为视频模型增加独立的视频开关，保留麦克风按钮；关闭视频停止采集和发送，
  不影响语音，并通过统一环境事件向模型同步视觉输入状态，区分历史画面与当前画面。
