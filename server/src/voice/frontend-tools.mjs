...
const spawnThinkingTool = {
  ...
  function: {
    ...
    description: '异步执行需要访问用户环境、设备、文件、屏幕、应用、代码、图片生成、其他媒体创作、继续、修改已有工作或多步办事能力的工作；这是前台 Agent 的统一复杂工作入口。公开网页搜索和网址读取优先使用前台专用工具。请求明确时直接调用，调用前不要口头回应，也不要先否认能力。本轮附件会自动传入；只在缺少处理意图时询问。多个独立目标可在同一响应中分别调用；不要重复提交已经覆盖的目标。查询已有工作的状态、进度或阶段结果改用 get_agent_task_status。accepted 表示本次提交成功；duplicate 表示同一目标此前已提交；二者都不表示完成。收到本次响应的全部回执后只自然确认一次，不向用户暴露后台 Agent、协议、Task 或 ID，也不再调用工具。',
    parameters: {
      type: 'object',
      properties: {
        objective: {
