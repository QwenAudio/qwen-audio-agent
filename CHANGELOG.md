...
- 新增 Windows 10/11 x64 + WSL2 桌面客户端测试版：对齐 macOS 悬浮球、设置、
  托盘、静音、拖动与更新体验；默认在所选 WSL2 发行版中安装并管理版本匹配的
  私有 Gateway 运行时，复用原有配置、Agent 登录、记忆和任务，并提供外部
  loopback Gateway 高级模式、开机启动、诊断、确认安装/移除及有界崩溃恢复。
  首版不支持 Windows ARM64、WSL1 或原生 Windows Gateway。
