...
const recallTool = {
  ...
  function: {
    ...
    description: '回忆此前发生过什么 —— 聊过哪些话题、派过哪些活、手上有哪些用户给的资料。用户问“我们之前聊过某事吗”“前几天说的那个”“上次让你做的那件事”“我给你的那份手册讲了什么”“最近都聊了什么”等回顾过去或查找已有资料的问题时调用。传入用户提到的关键词；泛泛问“最近怎么样”时省略 query。返回三类内容：sessions 是每场对话的话题、一句要点，以及那场派过的活及其当前状态；documents 是用户导入的资料，含标题、一句说明、章节标题和文件路径。都不含原文和执行细节。需要资料里的具体内容或要基于它做事，用 spawn_thinking，并在 objective 里带上返回的 path 和相关章节标题，让后台去读；想知道某项工作的详细进展或结果全文，改用 get_agent_task_status。返回 not_found 时如实说明没找到，不要编造聊过的内容，也不要凭标题猜测资料里写了什么。',
    parameters: {
      type: 'object',
      properties: {
        query: {
