...
- **后台执行层**由 `cockpit-agent` 示范。后台 Agent 可按需派生独立 Session，
  形成由第二层扩展出的第三层执行空间；当前轻量示例只做直接 MCP 调用，不实现
  派生 Session。
- `cockpit-domain` 是 Demo 为同时驱动 UI 和工具而提供的场景基础设施，不属于
...
| 目录 / 入口 | 性质 | 允许依赖 | 不应承担 |
|---|---|---|---|
| `react-app/` | 前台客户端示例 | GCP Client SDK、座舱 Domain HTTP/SSE | Realtime Provider、后台 Agent 实现、业务执行 |
| `gateway.mjs` | 前台装配入口 | qwen-audio-agent 公开导出、场景 Profile、A2A Agent Card | 复制 Gateway 核心、解析座舱业务对象 |
| `agent/` | 后台 Agent 示例 | A2A SDK、`/mcp/backend` | UI 控制、Realtime 会话、通用 Agent 框架 |
| `tools/` | 场景工具定义与执行器 | 显式 Registry、Domain 注入的状态和外部服务 | Gateway 协议、客户端渲染 |
| `domain/` | 场景业务基础设施 | `tools/`、高德适配、HTTP/SSE/MCP Transport | 对话、播报、Agent 编排 |
...
这里不存在“框架 WebUI”。`react-app` 是客户场景客户端的参考实现，它直接使用
