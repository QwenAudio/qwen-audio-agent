...
export class CockpitDomainServer {
  constructor({
    domain = new CockpitDomain({ services: createAmapCockpitServices() }),
    host = '127.0.0.1',
    port = 3010,
  } = {}) {
    this.domain = domain
  ...
  async #handle(request, response) {
    ...
    if (url.pathname === '/health' && request.method === 'GET') {
      json(response, 200, { ok: true, service: 'cockpit-domain' })
      return
    }
    if (url.pathname === '/api/cockpit/state' && request.method === 'GET') {
      json(response, 200, this.domain.snapshot(cockpitId(request, url)))
      return
    }
    ...
    if (url.pathname === '/api/cockpit/commands' && request.method === 'POST') {
      ...
      const output = await this.domain.execute(body.name, body.arguments || {}, {
...
export async function startCockpitDomainServer(options = {}) {
  return new CockpitDomainServer(options).start()
}
...
if (entry === import.meta.url) {
  ...
  console.log(`Cockpit domain service listening on ${server.origin}`)
  const close = async () => {
    await server.close()
    process.exit(0)
  }
