...
| `realtime.visual-input-buffer-v1` | Negotiated Web clients append bounded JPEG visual frames through GCP `input_image_buffer.append`; Gateway validates and throttles them while Qwen Omni and MiniCPM-o adapters own provider-native encoding | `test/gateway-client-protocol.test.mjs`, `server/test/visual-input-buffer.test.mjs`, `server/test/realtime-provider.test.mjs`, `server/test/minicpm-o-provider.test.mjs`, `web/test/camera-input.test.mjs` |
